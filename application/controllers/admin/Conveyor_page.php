...
class Conveyor_page extends CI_Controller {
	...
	public function action()
	{
		...
		elseif ($this->uri->segment(4) == 'conveyor6' ) {
			if (!$this->ion_auth->logged_in())
				{
					// redirect them to the login page
					redirect('admin/auth/login', 'refresh');
			...
			else
				{
					...
					$data['conveyor6'] = $this->M_data->tampil_to_form($user->id,3,6,'conveyor6')->result();
					$data['content'] = 'admin/conveyor/conveyor6';
					$this->load->view('admin/master', $data);
				}
				//echo $this->uri->segment(4);
		}elseif ($this->uri->segment(4) == 'conveyor7' ) {
			if (!$this->ion_auth->logged_in())
				{
					// redirect them to the login page
					redirect('admin/auth/login', 'refresh');
			...
			else
				{
					...
					$data['conveyor7'] = $this->M_data->tampil_to_form($user->id,3,7,'conveyor7')->result();
					$data['content'] = 'admin/conveyor/conveyor7';
					$this->load->view('admin/master', $data);
				}
				//echo $this->uri->segment(4);
		}else{
...
#################################################END DEBUGING UPLOAD ########################################################
        ...
        $file = $this->input->post("field");
			if ($user->id) {
				$this->session->set_flashdata('message', $this->ion_auth->messages());
					     ...
					     //$file1 = $_FILES["field2".$i]["name"];
			                ...
			                else
			                {
			                        $data = [
										...
										$this->M_data->input_data($data,'conveyor6');
										$this->session->set_flashdata('data','Data baru berhasil di input..');
					...
					redirect('admin/conveyor_page/action/conveyor6/', 'refresh');
	...
	public function conveyor6_edit(){
		$data['users'] = $this->ion_auth->users()->result();
		foreach ($data['users'] as $k => $user)
			{
				$data['users'][$k]->groups = $this->ion_auth->get_users_groups($user->id)->result();
			   	...
			   	if ( (!$this->upload->do_upload('field2')) )
	                ...
	                else
	                {
	                    $data = [
							...
							$input = $this->input->post("id");
							$data1['conveyor6'] = $this->M_data->tampil_data_controller($input,'conveyor6')->result();
					        foreach ($data1['conveyor6'] as $k => $file)
							{
								$data1['conveyor6'][$k]= $this->M_data->tampil_data_controller($input,'conveyor6')->result();
				            }
				            unlink("./upload/conveyor/$file->file");
							$this->M_data->update_data($id,$data,'conveyor6');
							$this->session->set_flashdata('data','Update data berhasil..');
		...
		public function conveyor6_remove(){
		$data['users'] = $this->ion_auth->users()->result();
		foreach ($data['users'] as $k => $user)
			{
				$data['users'][$k]->groups = $this->ion_auth->get_users_groups($user->id)->result();
		...
		if ($user->id) {
	        ...
	        foreach ($data1['conveyor6'] as $k => $file)
			{
				...
				if ($this->M_data->tampil_to_form($user->id,3,1,'conveyor6')->result()){
						$where = $input;
						unlink("./upload/conveyor/$file->file");
						$this->M_data->hapus_data($where,'conveyor6');
						$this->session->set_flashdata('data','Hapus data berhasil..');
				}
				redirect('admin/conveyor_page/action/conveyor6/', 'refresh');
		} else {
        ...
        $file = $this->input->post("field");
			if ($user->id) {
				$this->session->set_flashdata('message', $this->ion_auth->messages());
					     ...
					     //$file1 = $_FILES["field2".$i]["name"];
			                ...
			                else
			                {
			                        $data = [
										...
										$this->M_data->input_data($data,'conveyor7');
										$this->session->set_flashdata('data','Data baru berhasil di input..');
					...
					redirect('admin/conveyor_page/action/conveyor7/', 'refresh');
	...
	public function conveyor7_edit(){
		$data['users'] = $this->ion_auth->users()->result();
		foreach ($data['users'] as $k => $user)
			{
				$data['users'][$k]->groups = $this->ion_auth->get_users_groups($user->id)->result();
			   	...
			   	if ( (!$this->upload->do_upload('field2')) )
	                ...
	                else
	                {
	                    $data = [
							...
							$input = $this->input->post("id");
							$data1['conveyor7'] = $this->M_data->tampil_data_controller($input,'conveyor7')->result();
					        foreach ($data1['conveyor7'] as $k => $file)
							{
								$data1['conveyor7'][$k]= $this->M_data->tampil_data_controller($input,'conveyor7')->result();
				            }
				            unlink("./upload/conveyor/$file->file");
							$this->M_data->update_data($id,$data,'conveyor7');
							$this->session->set_flashdata('data','Update data berhasil..');
		...
		public function conveyor7_remove(){
		$data['users'] = $this->ion_auth->users()->result();
		foreach ($data['users'] as $k => $user)
			{
				$data['users'][$k]->groups = $this->ion_auth->get_users_groups($user->id)->result();
		...
		if ($user->id) {
	        ...
	        foreach ($data1['conveyor7'] as $k => $file)
			{
				...
				if ($this->M_data->tampil_to_form($user->id,3,7,'conveyor7')->result()){
						$where = $input;
						unlink("./upload/conveyor/$file->file");
						$this->M_data->hapus_data($where,'conveyor7');
						$this->session->set_flashdata('data','Hapus data berhasil..');
				}
				redirect('admin/conveyor_page/action/conveyor7/', 'refresh');
		} else {
